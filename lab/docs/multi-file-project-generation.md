...
## Реализация: functions/project_gen.py (проверено в UI)
Pipe-«модель» `project_gen`. Один промпт → проект как настоящие скачиваемые файлы. Проверено
end-to-end в браузере: карточки файлов + `cli_calculator.zip` + ссылка «Скачать весь проект» +
дерево; zip скачивается и разворачивается в `cli_calculator/main.py`, `parser.py`, `operations.py`
с чистым кодом; отдельный файл отдаётся `content-type: text/x-python`, `Content-Disposition: attachment`.

Грабли, которые всплыли и учтены:
- **Reasoning-модель пишет рассуждение в манифест** и эхом повторяет пример из системного промпта.
  Нельзя брать первый `{...}`. Экстрактор `_extract_manifest` сканирует по балансу скобок и берёт
  ПОСЛЕДНИЙ объект, который парсится И содержит непустой `files`; плюс ретрай с более жёстким
  промптом.
- **Та же протечка в теле файла:** без парсера reasoning уходит в контент и проедает бюджет
  (файл на 70 КБ из одного рассуждения, отсюда и медленно). Фикс двойной: системный промпт
  «отвечай сразу кодом, первый символ = первая строка файла» + `_clean_code` срезает всё до
  первой кодовой строки (`import/from/def/class/#/...`) и снимает `<think>`.
- Скорость: генерация по файлу с автопродолжением медленная (минуты на проект), потому что каждый
  вызов идёт в reasoning-слаг. Дефолты сжаты (MAX_FILES=5, FILE_MAX_TOKENS=6000, CONT_ROUNDS=2).
- Тест-артефакт: чистка файлов в БД НЕ отменяет уже идущую серверную генерацию, старый ран
  дописывает файлы после clean. Для чистого прогона дождаться тишины по числу файлов.

Открытое: качество согласованности между файлами (импорты) держится на сигнатурах в контексте
и не гарантировано; финальный проход «проверь импорты» пока не добавлен.
